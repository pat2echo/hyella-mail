<?php
	 ...
	 * complaints Class
	 *
	 * @used in  				complaints Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	complaints
	 */

	/*
	|--------------------------------------------------------------------------
	| complaints Function in Settings Module
	|--------------------------------------------------------------------------
	|
	| Interfaces with database table to generate data capture form, dataTable,
	| execute search, insert new records into table,
    delete and modify existing
	| in the dataTable.
	|
	*/
	...
	class cComplaints{
		...
		private $table_fields = array(
			'title' => 'complaints001',
			'description' => 'complaints002',
			'category' => 'complaints003',
			'priority' => 'complaints004',
			'attached_file' => 'complaints005',
			'status' => 'complaints006',
		);
		...
		private $datatable_settings = array(
			'show_toolbar' => 1,				//Determines whether or not to show toolbar [Add New | Advance Search | Show Columns will be displayed]
                ...
                'custom_edit_button' => '<a href="#" class="custom-single-selected-record-button btn btn-sm btn-danger" function-id="1" search-table="" function-class="complaints" function-name="reply_complaint" module-id="" module-name="" action="?module=&action=complaints&todo=reply_complaint" mod="edit-complaints" todo="reply_ticket">Reply</a>',		//Determines whether or not to show edit button
		...
		function complaints(){
			...
			break;
			case 'get_complaints':
				$returned_value = $this->_get_complaints();
			break;
			case 'reply_complaint':
				$returned_value = $this->_get_reply_complaint_form();
			break;
			}
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'ccomplaints.php';
		...
		private function _get_reply_complaint_form(){
            //GENERATE REPLY COMPLAINT FORM
            $this->class_settings['form_heading_title'] = 'Kayode';
            
            $this->class_settings['hidden_records'] = array(
                $this->table_fields[ 'title' ] => 1,
                $this->table_fields[ 'description' ] => 1,
                $this->table_fields[ 'category' ] => 1,
                $this->table_fields[ 'priority' ] => 1,
                $this->table_fields[ 'attached_file' ] => 1,
            );
            return $this->_generate_new_data_capture_form();
        }
        
		private function _get_complaints(){
			...
			$all_complaints = execute_sql_query($query_settings);
			
			$complaints = array();
			
			if( is_array( $all_complaints ) && ! empty( $all_complaints ) ){
				
				foreach( $all_complaints as $category ){
					$complaints[ 'all' ][ $category[ 'id' ] ] = $category['title'];
					$complaints[ $category[ 'id' ] ] = $category;
				}
				
				//Cache Settings
				$settings = array(
					'cache_key' => $cache_key,
					'cache_values' => $complaints,
					'permanent' => true,
				);

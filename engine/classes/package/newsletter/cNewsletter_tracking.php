<?php
	/**
	 * newsletter_tracking Class
	 *
	 * @used in  				newsletter_tracking Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	newsletter_tracking
	 */
	
	class cNewsletter_tracking{
		public $class_settings = array();
		
		private $current_record_id = '';
		
		public $table_name = 'newsletter_tracking';
		...
		private $table_fields = array(
			'date' => 'newsletter_tracking001',
			'email' => 'newsletter_tracking002',
			'action' => 'newsletter_tracking003',
			'message' => 'newsletter_tracking004',
			'ip' => 'newsletter_tracking005',
		);
		...
		private function _save_new_record_popup_form(){
			...
			if( isset( $return["saved_record_id"] ) && $return["saved_record_id"] ){
				...
				$title = "<h4>New Item newsletter_tracking Successfully Created</h4>";
				switch( $this->class_settings["action_to_perform"] ){
				default:
					$title =  '<p>New newsletter_tracking: <strong>' . ( isset( $c[ "id" ] )?$c[ "name" ]:"N/A" ) . '</strong> Successfully Created</p>';
				break;
				}
				...
				default:
					...
					$return[ "html_append_selector" ] = 'select[name="newsletter_tracking"]';
		...
		private function _new_record_popup_form(){
			...
			$this->class_settings[ 'data' ]["html_title"] = "Create Newsletter Tracking";
		...
		private function _delete_app_record(){
			if( isset( $_POST['id'] ) && $_POST['id'] ){
				...
				if( isset( $return['deleted_record_id'] ) && $return['deleted_record_id'] ){
					...
					$return["javascript_functions"] = array( "nwnewsletter_tracking.emptyNewItem" );
					return $return;
				}
			}
		}
		...
		private function _save_app_changes(){
			$return = array();
			$js = array( 'nwnewsletter_tracking.init' );
			$new = 0;
			if( isset( $_POST['id'] ) ){
				...
				if( ! $_POST['id'] ){
					//new mode
					$js[] = 'nwnewsletter_tracking.reClick';
					$new = 1;
				}
				...
				if( isset( $return['saved_record_id'] ) && $return['saved_record_id'] ){
					...
					$e = $this->_get_newsletter_tracking();
					
					if( isset( $e[ $return['saved_record_id'] ]["id"] ) ){
						$this->class_settings[ 'data' ]["item"] = $e[ $return['saved_record_id'] ];
						$this->class_settings[ 'html' ] = array( 'html-files/templates-1/package/'.HYELLA_PACKAGE.'/'.$this->table_name.'/expense-list.php' );
						$returning_html_data = $this->_get_html_view();
		...
		private function _display_app_view(){
			...
			$this->class_settings[ 'data' ]['newsletter_tracking'] = $this->_get_all_newsletter_tracking();
			$this->class_settings[ 'data' ]['type'] = get_product_types();
			
			$this->class_settings[ 'html' ] = array( 'html-files/templates-1/package/'.HYELLA_PACKAGE.'/'.$this->table_name.'/display-app-view' );
			$returning_html_data = $this->_get_html_view();
		...
		private function _get_all_newsletter_tracking(){
			$select = "";
			foreach( $this->table_fields as $key => $val ){
		...
		private function _display_all_records_full_view(){
			...
			$this->class_settings[ 'data' ]['title'] = "Manage Tracking";
		...
		private function _generate_new_data_capture_form(){
			...
			switch ( $this->class_settings['action_to_perform'] ){
			case 'edit':
				$this->class_settings['form_heading_title'] = 'Modify Tracking Info';
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cnewsletter_tracking.php';
		...
		private function _save_changes(){
			...
			if( is_array( $returning_html_data ) ){
				...
				if( isset( $returning_html_data['saved_record_id'] ) && $returning_html_data['saved_record_id'] ){
					...
					$record = $this->_get_newsletter_tracking();
				}
			}
		...
		private function _get_newsletter_tracking(){
			...
			if( is_array( $all_departments ) && ! empty( $all_departments ) ){
				...
				foreach( $all_departments as $newsletter_tracking ){
					$departments[ 'all' ][ $newsletter_tracking[ 'id' ] ] = $newsletter_tracking['name'];
					
					$c = ( isset( $cs[ $newsletter_tracking[ 'type' ] ] )?$cs[ $newsletter_tracking[ 'type' ] ]:$newsletter_tracking[ 'type' ] );
					$departments[ 'group' ][ $c ][ $newsletter_tracking[ 'id' ] ] = $newsletter_tracking[ 'name' ];
					
					$departments[ $newsletter_tracking[ 'id' ] ] = $newsletter_tracking;
				}
		...
		private function _reset_members_cache( $record , $clear = 0 ){
			...
			$settings = array(
				'cache_key' => $cache_key.'-newsletter_tracking-',//.$record["member_id"],
				'directory_name' => $cache_key,
				'permanent' => true,
			);

<?php
	 ...
	 * pricing Class
	 *
	 * @used in  				pricing Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	pricing
	 */

	/*
	|--------------------------------------------------------------------------
	| pricing Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cPricing{
		...
		private $table_fields = array(
			'price_category' => 'pricing001',
			'specific_month' => 'pricing002',
			'specific_day' => 'pricing003',
			'price' => 'pricing004',
		);
		...
		private function _display_all_records_full_view(){
			...
			$this->class_settings[ 'data' ]['title'] = "Manage Hall Pricing";
		...
		private function _generate_new_data_capture_form(){
			...
			switch ( $this->class_settings['action_to_perform'] ){
			case 'edit':
				$this->class_settings['form_heading_title'] = 'Modify Hall Pricing';
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cpricing.php';
		...
		private function _get_pricing(){
			...
			if( is_array( $all_departments ) && ! empty( $all_departments ) ){
				...
				foreach( $all_departments as $category ){
					$departments[ $category[ 'specific_month' ] . '-' . $category[ 'specific_day' ] ] = $category;
		...
		private function _reset_members_cache( $record , $clear = 0 ){
			...
			$settings = array(
				'cache_key' => $cache_key.'-pricing-',//.$record["member_id"],
				'directory_name' => $cache_key,
				'permanent' => true,
			);

	...
	*/
	...
	class cScript_compiler{
				...
				//if( ! file_exists( $filename ) || $this->class_settings[ 'project_data' ][ 'development_mode' ] ){
					...
					foreach( $this->class_settings[ 'css' ] as $file_link ){
						if( file_exists( $this->class_settings[ 'calling_page' ] . $file_link ) ){
                            ...
                            $css_file_content .= '<link type="text/css" href="' . $this->class_settings[ 'calling_page_display' ] . $file_link . '" rel="stylesheet" />';
		...
		private function _html_markup(){
			...
			if( isset( $this->class_settings[ 'priv_id' ] ) ){
			...
			if( isset( $this->class_settings[ 'html' ] ) && is_array( $this->class_settings[ 'html' ] ) ){
				...
				foreach( $this->class_settings[ 'html' ] as $file_link ){
					if( file_exists( $this->class_settings[ 'calling_page' ] . $file_link ) && is_file( $this->class_settings[ 'calling_page' ] . $file_link ) ){
						//READ CONTENTS OF EACH FILE
						ob_start();
						...
						if( file_exists( $this->class_settings[ 'calling_page' ] . $file_link . '/index.php' ) ){
							//READ CONTENTS OF EACH FILE
							ob_start();
							include( $this->class_settings[ 'calling_page' ] . $file_link . '/index.php' );
							$html_file_content .= ob_get_contents();
							ob_end_clean();
						}
					}

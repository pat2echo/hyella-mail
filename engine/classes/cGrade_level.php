<?php
	 ...
	 * grade_level Class
	 *
	 * @used in  				grade_level Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	grade_level
	 */

	/*
	|--------------------------------------------------------------------------
	| grade_level Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cGrade_level{
		...
		private $table_fields = array(
			'grade_level' => 'grade_level001',
			'step' => 'grade_level002',
			'basic_salary' => 'grade_level003',
			'housing' => 'grade_level004',
			'child_bonus' => 'grade_level005',
			'dependent_bonus' => 'grade_level006',
			'medical_allowance' => 'grade_level007',
			'transport' => 'grade_level008',
			'utility' => 'grade_level009',
			'lunch' => 'grade_level010',
			
			'salary_schedule' => 'grade_level012',
		);
		...
		function grade_level(){
			...
			break;
			case 'get_all_grade_level':
				$returned_value = $this->_get_all_grade_level();
			break;
			case 'refresh_cache':
		...
		private function _refresh_cache(){
			...
			$this->_get_grade_level();
		}
		
		private function _get_all_grade_level(){
			$select = "";
			...
			$query = "SELECT ".$select." FROM `".$this->class_settings['database_name']."`.`".$this->table_name."` WHERE `".$this->table_name."`.`record_status` = '1' ORDER BY `".$this->table_name."`.`".$this->table_fields["grade_level"]."` ";
		...
		private function _generate_new_data_capture_form(){
			...
			switch ( $this->class_settings['action_to_perform'] ){
			case 'edit':
				$this->class_settings['form_heading_title'] = 'Modify Grade Level';
			break;
			}
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cgrade_level.php';
		...
		private function _get_grade_level(){
			...
			$query = "SELECT ".$select." FROM `" . $this->class_settings['database_name'] . "`.`".$this->table_name."` where `record_status`='1' ORDER BY `".$this->table_fields["grade_level"]."`, `".$this->table_fields["step"]."` ";
			...
			if( is_array( $all_departments ) && ! empty( $all_departments ) ){
				...
				foreach( $all_departments as $grade_level ){
					$departments[ 'all' ][ $grade_level[ 'id' ] ] = $grade_level['grade_level'] . ' ( ' .$grade_level['step'] . ' ) ';
					$departments[ $grade_level[ 'id' ] ] = $grade_level;
				}
		...
		private function _reset_members_cache( $record , $clear = 0 ){
			...
			$settings = array(
				'cache_key' => $cache_key.'-grade_level-',//.$record["member_id"],
				'directory_name' => $cache_key,
				'permanent' => true,
			);

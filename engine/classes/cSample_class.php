	...
	*/
	...
	class cVisit_schedule{
		...
		private function _get_visit_schedule(){
			...
			if( is_array( $all_visit_schedule ) && ! empty( $all_visit_schedule ) ){
				
				foreach( $all_visit_schedule as $category ){
					$visit_schedule[ $category[ 'id' ] ] = $category;
				}
				
				//Cache Settings
				$settings = array(
					'cache_key' => $cache_key,
					'cache_values' => $visit_schedule,
					'permanent' => true,
				);

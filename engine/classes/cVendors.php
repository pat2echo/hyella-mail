<?php
	 ...
	 * vendors Class
	 *
	 * @used in  				vendors Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	vendors
	 */

	/*
	|--------------------------------------------------------------------------
	| vendors Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cVendors{
		...
		function vendors(){
			...
			break;
			case 'search_vendor':
				$returned_value = $this->_search_vendor();
			break;
			case "new_source_popup_form":
			case "new_vendors_popup_form":
				$returned_value = $this->_new_vendors_popup_form();
			break;
			case "save_new_source_popup":
			case "save_new_vendors_popup":
				$returned_value = $this->_save_new_vendors_popup();
			break;
			case "view_vendor_details":
				$returned_value = $this->_view_vendor_details();
			break;
			}
		...
		private function _view_vendor_details(){
			if( ! ( isset( $_POST["id"] ) && $_POST["id"] ) ){
				...
				$err->additional_details_of_error = 'Invalid Vendor';
				return $err->error();
			}
			$_POST["vendor"] = $_POST["id"];
			
			$html = $this->_search_vendor();
			...
			$this->class_settings[ 'data' ]["html_title"] = "Vendor Details";
		...
		private function _save_new_vendors_popup(){
			...
			if( isset( $return["saved_record_id"] ) && $return["saved_record_id"] ){
				...
				$err->additional_details_of_error = '<h4>New Vendor Successfully Created</h4>';
				$return = $err->error();
				...
				$return[ "html_append" ] = '<option selected="selected" value="'. $id .'">'. ( isset( $c[ $id ] )?$c[ $id ]:"N/A" ) .'</option>';
				
				$return[ "html_append_selector" ] = 'select[name="vendor"]';
				
				switch( $this->class_settings["action_to_perform"] ){
				case "save_new_source_popup":
					$return[ "html_append_selector" ] = 'select[name="source"]';
				break;
				}
		...
		private function _new_vendors_popup_form(){
			
			$this->class_settings[ 'form_action' ] = '?action='.$this->table_name.'&todo=save_new_vendors_popup';
			
			switch( $this->class_settings["action_to_perform"] ){
			case "new_source_popup_form":
				$this->class_settings[ 'form_action' ] = '?action='.$this->table_name.'&todo=save_new_source_popup';
			break;
			}
			...
			$this->class_settings[ 'html' ] = array( 'html-files/templates-1/'.$this->table_name.'/zero-out-negative-budget' );
			$this->class_settings[ 'data' ]["html_title"] = "Create New Vendor";
			$this->class_settings[ 'data' ]['html'] = $html;
		...
		private function _search_vendor(){
			$where = "";
			...
			$where = " `".$this->table_name."`.`record_status` = '1' AND `id` = '".$_POST["vendor"] . "' ";
			...
			if( $where ){
				...
				if( ! empty( $customer ) ){
					...
					$inventory->class_settings[ "vendor" ] = $customer[0][ "id" ];
					$inventory->class_settings[ "action_to_perform" ] = 'get_all_inventory_by_vendor';
					
					$this->class_settings[ 'data' ][ "transaction" ] = $inventory->inventory();
					$this->class_settings[ 'data' ][ "vendor" ] = $customer[0];
					*/
					$transactions = new cTransactions();
					$transactions->class_settings = $this->class_settings;
					$transactions->class_settings[ "account_type" ] = "account_payable";
					...
					$this->class_settings[ 'html' ] = array( 'html-files/templates-1/'.$this->table_name.'/vendor-info.php' );
					$returning_html_data = $this->_get_html_view();
					...
					$inventory->class_settings[ "action_to_perform" ] = 'search_vendor_inventory_record';
					$sales_record = $inventory->inventory();
					
					return array(
						'html_replacement_selector' => "#sales-record-search-result",
						'html_replacement' => $returning_html_data,
						
						'html_replacement_selector_one' => "#vendors-transactions",
						'html_replacement_one' => isset( $sales_record['html_replacement'] )?$sales_record['html_replacement']:"",
						
						'method_executed' => $this->class_settings['action_to_perform'],
						'status' => 'new-status',
						'javascript_functions' => array( 'nwVendors.activateTabs' ),
					);
				}else{
					$err = new cError(010014);
					$err->action_to_perform = 'notify';
		...
		private function _delete_app_record(){
			if( isset( $_POST['id'] ) && $_POST['id'] ){
				...
				if( isset( $return['deleted_record_id'] ) && $return['deleted_record_id'] ){
					...
					$return["javascript_functions"] = array( "nwVendors.emptyNewItem" );
					return $return;
			...
			if( isset( $_POST['id'] ) ){
				...
				if( isset( $return['saved_record_id'] ) && $return['saved_record_id'] ){
					...
					if( isset( $e[ $return['saved_record_id'] ]["id"] ) ){
						...
						switch( $this->class_settings[ 'action_to_perform' ] ){
						case 'save_manage_app_changes':
							$this->class_settings[ 'data' ]["vendor"] = $e[ $return['saved_record_id'] ];
							$this->class_settings[ 'html' ] = array( 'html-files/templates-1/'.$this->table_name.'/vendor-info.php' );
							$js = array();
						break;
						default:
		...
		private function _display_app_view(){
			...
			break;
			default:
				$this->class_settings[ 'data' ]['vendors'] = $this->_get_all_vendors();
		...
		private function _get_all_vendors(){
			$select = "";
			foreach( $this->table_fields as $key => $val ){
				if( $select )$select .= ", `".$this->table_name."`.`".$val."` as '".$key."'";
				else $select = " `".$this->table_name."`.`id`, `".$this->table_name."`.`serial_num`, `".$this->table_name."`.`".$val."` as '".$key."'";
			...
			$query = "SELECT ".$select." FROM `".$this->class_settings['database_name']."`.`".$this->table_name."` WHERE `".$this->table_name."`.`record_status` = '1' ORDER BY `".$this->table_name."`.`".$this->table_fields["name_of_vendor"]."` ";
		...
		private function _display_all_records_full_view(){
			...
			$this->class_settings[ 'data' ]['title'] = "Manage Vendors / Suppliers";
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cvendors.php';
		...
		private function _get_vendors(){
			...
			$query = "SELECT ".$select." FROM `" . $this->class_settings['database_name'] . "`.`".$this->table_name."` where `record_status`='1' ORDER BY `".$this->table_fields["name_of_vendor"]."`";
			...
			if( is_array( $all_departments ) && ! empty( $all_departments ) ){
				...
				foreach( $all_departments as $category ){
					$departments[ 'all' ][ $category[ 'id' ] ] = $category['name_of_vendor'] . " (".$category['address'].")";
		...
		private function _reset_members_cache( $record , $clear = 0 ){
			...
			$settings = array(
				'cache_key' => $cache_key.'-vendors-',//.$record["member_id"],
				'directory_name' => $cache_key,
				'permanent' => true,
			);

	...
	*/
	...
	class cBanks{
		...
		private function _display_all_records_full_view(){
			...
			$this->class_settings[ 'data' ]['title'] = "Manage Banks";
		...
		private function _generate_new_data_capture_form(){
			...
			switch ( $this->class_settings['action_to_perform'] ){
			case 'edit':
				$this->class_settings['form_heading_title'] = 'Modify Banks';
				//$this->class_settings['hidden_records'][ $this->table_fields["member_id"] ] = 1;
			break;
			}
			
			//$this->class_settings['hidden_records'][ $this->table_fields["number_of_animals_present"] ] = 1;
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cbanks.php';
		...
		private function _get_banks(){
			...
			if( is_array( $all_departments ) && ! empty( $all_departments ) ){
				...
				foreach( $all_departments as $category ){
					$departments[ 'all' ][ $category[ 'id' ] ] = $category['name_of_bank'] . " - " . $category['location'];

<?php
	 ...
	 *
	 * @used in  				customer_wish_list Function
	 * @created  				08:22 | 06-06-2016
	 * @database table name   	customer_wish_list
	 */
	
	class cCustomer_wish_list{
		public $class_settings = array();
		
		private $current_record_id = '';
		
		public $table_name = 'customer_wish_list';
		...
		private $table_fields = array(
			'date' => 'customer_wish_list001',
			'customer' => 'customer_wish_list002',
			'item' => 'customer_wish_list003',
			'description' => 'customer_wish_list004',
			'date_needed' => 'customer_wish_list005',
			'comment' => 'customer_wish_list006',
		);
		...
		function customer_wish_list(){
			...
			break;
			case 'search_customer_wish_list':
				$returned_value = $this->_search_customer_wish_list();
			break;
			}
		...
		private function _search_customer_wish_list(){
			...
			$data = $this->_get_all_customer_wish_list();
			...
			$return["html_replacement_selector"] = "#customer-wish-list-container";
		...
		private function _new_popup_form(){
			...
			$this->class_settings[ "form_values_important" ][ $this->table_fields[ "date_needed" ] ] = date("U");
			...
			$this->class_settings[ 'data' ]["html_title"] = $c_name.": New Wish List";
		...
		private function _delete_app_record(){
			if( isset( $_POST['id'] ) && $_POST['id'] ){
				...
				if( isset( $return['deleted_record_id'] ) && $return['deleted_record_id'] ){
					...
					$return["javascript_functions"] = array( "nwcustomer_wish_list.emptyNewItem" );
					return $return;
		...
		private function _save_app_changes(){
			$return = array();
			$js = array( 'nwcustomer_wish_list.init' );
			$new = 0;
			...
			if( isset( $_POST['id'] ) ){
				...
				if( $no_skip ){
					...
					if( ! $_POST['id'] ){
						//new mode
						$js[] = 'nwcustomer_wish_list.reClick';
						$new = 1;
					}
				...
				if( isset( $return['saved_record_id'] ) && $return['saved_record_id'] ){
					...
					$e = $this->_get_customer_wish_list();
		...
		private function _display_app_view(){
			...
			$this->class_settings[ 'data' ]['customer_wish_list'] = $this->_get_all_customer_wish_list();
		...
		private function _get_all_customer_wish_list(){
			$select = "";
			foreach( $this->table_fields as $key => $val ){
		...
		private function _display_all_records_full_view(){
			...
			$this->class_settings[ 'data' ]['title'] = "Manage Customer Wish List";
		...
		private function _generate_new_data_capture_form(){
			...
			switch ( $this->class_settings['action_to_perform'] ){
			case 'edit':
				$this->class_settings['form_heading_title'] = 'Modify Wish List';
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'ccustomer_wish_list.php';
		...
		private function _save_changes(){
			...
			if( is_array( $returning_html_data ) ){
				...
				if( isset( $returning_html_data['saved_record_id'] ) && $returning_html_data['saved_record_id'] ){
					...
					$record = $this->_get_customer_wish_list();
				}
			}
		...
		private function _get_customer_wish_list(){
			...
			foreach( $this->table_fields as $key => $val ){
				...
				else $select = "`id`, `serial_num`, `created_by`, `modified_by`, `".$val."` as '".$key."'";
			}

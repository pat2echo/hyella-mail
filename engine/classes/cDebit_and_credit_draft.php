<?php
	/**
	 * debit_and_credit_draft Class
	 *
	 * @used in  				debit_and_credit_draft Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	debit_and_credit_draft
	 */

	/*
	|--------------------------------------------------------------------------
	| debit_and_credit_draft Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cDebit_and_credit_draft{
		public $class_settings = array();
		
		private $current_record_id = '';
		
		public $table_name = 'debit_and_credit_draft';
		...
		public $table_fields = array(
			'transaction_id' => 'debit_and_credit_draft001',
			'account' => 'debit_and_credit_draft002',
			'amount' => 'debit_and_credit_draft003',
			'type' => 'debit_and_credit_draft004',
			
			'account_type' => 'debit_and_credit_draft005',
			'extra_reference' => 'debit_and_credit_draft006',
			'currency' => 'debit_and_credit_draft007',
			'comment' => 'debit_and_credit_draft008',
		);
		...
		function debit_and_credit_draft(){
			...
			break;
			case 'get_specific_debit_and_credit_draft':
			case 'view_all_debit_and_credit_draft_editable':
				$returned_value = $this->_view_all_debit_and_credit_draft();
			break;
			case 'save_debit_and_credit_draft':
				$returned_value = $this->_save_debit_and_credit_draft();
			break;
			case 'delete_debit_and_credit_draft':
				$returned_value = $this->_delete_debit_and_credit_draft();
			break;
			case 'delete_items':
		...
		private function _refresh_cache(){
			...
			$this->_get_debit_and_credit_draft();
		}
		
		private function _save_debit_and_credit_draft(){
			
			if( ! ( isset( $this->class_settings["debit_and_credit_draft"] ) && is_array( $this->class_settings["debit_and_credit_draft"] ) ) ){
				return 0;
			}
			if( ! ( isset( $this->class_settings[ 'transaction_id' ] ) ) ){
				return 0;
			...
			foreach( $this->class_settings["debit_and_credit_draft"] as $k => $v ){
		...
		private function _delete_debit_and_credit_draft(){
			...
			$return["html_removal"] = "#debit_and_credit_draft-" . $this->class_settings[ 'current_record_id' ];
			
			unset( $return["html"] );
			return $return;
		}
		
		private function _save_new_debit_and_credit_draft(){
			//check for duplicate record
			$edit_mode = 0;
			...
			if( isset( $return['saved_record_id'] ) && $return['saved_record_id'] ){
				...
				$record = $this->_get_debit_and_credit_draft();
				...
				if( $edit_mode ){
					...
					$return["html_replace_selector"] = "#debit_and_credit_draft-".$record[ 'id' ];
				}else{							
					$return["html_prepend"] = $this->_get_html_view();
					$return["html_prepend_selector"] = "#form-control-table-debit_and_credit_draft";
				}
		...
		private function _view_all_debit_and_credit_draft(){
			...
			case 'view_all_debit_and_credit_draft_editable':
				...
				$this->class_settings[ 'form_action_todo' ] = 'save_new_debit_and_credit_draft';
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cdebit_and_credit_draft.php';
		...
		private function _get_debit_and_credit_draft(){
		...
		private function _clear_members_cache( $id ){
			...
			$settings = array(
				'cache_key' => $cache_key.'-debit_and_credit_draft-'.$id,
				'directory_name' => $cache_key,
				'permanent' => true,
			);
		...
		private function _reset_members_cache( $record , $clear = 0 ){
			...
			$settings = array(
				'cache_key' => $cache_key.'-debit_and_credit_draft-'.$record["transaction_id"],
				'directory_name' => $cache_key,
				'permanent' => true,
			);

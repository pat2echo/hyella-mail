<?php
	/**
	 * other_requirements Class
	 *
	 * @used in  				other_requirements Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	other_requirements
	 */

	/*
	|--------------------------------------------------------------------------
	| other_requirements Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cOther_requirements{
		public $class_settings = array();
		
		private $current_record_id = '';
		
		public $table_name = 'other_requirements';
		...
		private $table_fields = array(
			'description' => 'other_requirements001',
			'price' => 'other_requirements002',
			
			'unit' => 'other_requirements003',
			'category' => 'other_requirements004',
			
			'comment' => 'other_requirements005',
			'photograph' => 'other_requirements006',
		);
		...
		private function _display_all_records_full_view(){
			...
			$this->class_settings[ 'data' ]['title'] = "Manage Other Items Pricing";
		...
		private function _generate_new_data_capture_form(){
			...
			switch ( $this->class_settings['action_to_perform'] ){
			case 'edit':
				$this->class_settings['form_heading_title'] = 'Modify Other Items Price';
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cother_requirements.php';
		...
		private function _save_changes(){
			...
			if( is_array( $returning_html_data ) ){
				...
				if( isset( $returning_html_data['saved_record_id'] ) && $returning_html_data['saved_record_id'] ){
					...
					$record = $this->_get_other_requirements();
				}
			}
		...
		private function _get_other_requirements(){
			...
			$query = "SELECT ".$select." FROM `" . $this->class_settings['database_name'] . "`.`".$this->table_name."` where `record_status`='1' ORDER BY `".$this->table_fields["description"]."` ";
			...
			if( is_array( $all_departments ) && ! empty( $all_departments ) ){
				...
				foreach( $all_departments as $category ){
					...
					$c = isset( $cs[ $category[ 'category' ] ] )?$cs[ $category[ 'category' ] ]:$category[ 'category' ];
					$departments[ 'group' ][ $c ][ $category[ 'id' ] ] = $category[ 'description' ];
		...
		private function _reset_members_cache( $record , $clear = 0 ){
			...
			$settings = array(
				'cache_key' => $cache_key.'-other_requirements-',//.$record["member_id"],
				'directory_name' => $cache_key,
				'permanent' => true,
			);

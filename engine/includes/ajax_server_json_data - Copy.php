	...
	for($count_records=0; $count_records<count($arr); $count_records++ )
	{
		...
		if($skip_loop){
			...
			if($json_settings['show_details']){
				...
				$returning_html_data = '<button href="#" class="datatables-details btn '.$future_request.' remove-before-export" title="Click to View Details" jid="'.$aRow['id'].'" ><span class="budgetet"></span></button>';

var import_newsletter = {
	form_id:"import_newsletter-form",
	row_select_field:"import_newsletter008",
	col_select_field:"column-select-field",
	
};

	//hide all elements
	$("#"+import_newsletter.form_id)
	.find('.control-group')
	.addClass('hide-group');
	
	//create next button
	var $clone = $("#"+import_newsletter.form_id).find('.bottom-row').clone();
	$clone
	...
	$clone
	.insertAfter( $("#"+import_newsletter.form_id).find('.bottom-row') );
	...
	.bind('click', function(e){
		...
		if( $next.hasClass('form-group') ){
			...
			if( $v.hasClass( import_newsletter.col_select_field ) ){
			...
			if( p == true ){
				$("#"+import_newsletter.form_id).submit();
			}
		}
	...
	//set max & min limit
	$( "#" + import_newsletter.row_select_field )
	.attr('min', 1 )
	...
	//set max & min limit
	$( "input." + import_newsletter.col_select_field )
	.attr('min', 1 )
	...
	$('input.import-row-'+$( "#" + import_newsletter.row_select_field ).val() ).attr( 'checked', 'checked' );
	
	$('input[name="starting_row"]')
	.bind('change', function(){
		if( $( "#" + import_newsletter.row_select_field ).is(":visible") ){		
			hightlight_selected_row( $(this) );
			
			//update row_select_field if different
			if( $(this).val() != $( "#" + import_newsletter.row_select_field ).val() ){
				$( "#" + import_newsletter.row_select_field ).val( $(this).val() );
			}
		}
	});
	...
	.bind('change', function(){
		if( $( "input." + import_newsletter.col_select_field ).is(":visible") ){		
			hightlight_selected_col( $(this) );
			
			//update row_select_field if different
			if( $(this).val() != $( "input." + import_newsletter.col_select_field ).filter(":visible").val() ){
				$( "input." + import_newsletter.col_select_field ).filter(":visible").val( $(this).val() );
			}
		}
	});
...
function hightlight_selected_col( $e ){
	...
	$( "th.col-"+$e.val() )
	.find('.col-label')
	.text( $( "input." + import_newsletter.col_select_field ).filter(":visible").attr('col-label') );
};

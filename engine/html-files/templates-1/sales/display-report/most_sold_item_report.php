...
<?php
	...
	if( ! empty( $report_data ) ){
		...
		foreach( $report_data as $sval ){
			if( isset( $sval["payment_method"] ) && $sval["payment_method"] == "cash_refund" )continue;
			
			if( ! isset( $r[ $sval["item"] ] ) ){
				$r[ $sval["item"] ] = $sval;
				$r1[ $sval["item"] ] = $sval["quantity_sold"];
				//$r1[ $sval["item"] ] = $sval["amount_paid"];
			}
			else{
				foreach( $sval as $k => $v ){
					switch( $k ){
		...
		foreach( $r1 as $key => $val ){
			...
			$body .= '<tr>';
				...
				$body .= '<td class="company alternate">'.number_format( $sval["quantity_sold"] , 0 ). '</td>';
				
				$body .= '<td>' . number_format( $sval["cost"], 2 ) . '</td>';
	...
	<tr>
		<th class="alternate">Units of Goods Sold</th>

	...
	return {
		...
		init: function () {
			...
			.on('submit', function(e){
				...
				var account = $(this).find('select[name="account"]').val();
				var account_text1 = $(this).find('select[name="account"]').select2( 'data' );
				var account_text = account_text1.text;
				
				var account_type = account;
				
				if( $(this).find(".s2-account-select-2").is(":visible") && $(this).find('select[name="account2"]').val() && $(this).find('select[name="account2"]').val().length > 1 ){

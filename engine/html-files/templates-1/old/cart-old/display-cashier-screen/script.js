		...
		init: function () {			
			...
			.on('click', function(e){
				...
				if( $(this).attr("status") && parseFloat( $(this).attr("status") ) ){
					var data = {theme:'alert-info', err:'Transaction has been fully paid', msg:'The transaction you are trying to select has been fully paid for', typ:'jsuerror' };
					...
					nwRecordPayment.cartItems.customer = $item.attr("data-customer");
					
					if( $item.attr("data-reference_table") )
						nwRecordPayment.cartItems.reference_table = $item.attr("data-reference_table");
					
					if( $item.attr("data-extra_reference") )
						nwRecordPayment.cartItems.extra_reference = $item.attr("data-extra_reference");

var nwGroupCheckIn = function () {
	return {
		...
		init: function () {
			...
			.on("change", function(){
				...
				if( $(this).find("option[value='"+$(this).val()+"']").attr("data-type") ){
					...
					case 'percentage':
						nwGroupCheckIn.cartItems.discount_percentage = parseFloat( $(this).val() ) * 1;
						nwGroupCheckIn.cartItems.discount = 0;
					break;
					default:
						nwGroupCheckIn.cartItems.discount_percentage = 0;
						nwGroupCheckIn.cartItems.discount = parseFloat( $(this).val() ) * 1;
					break;
					}
				}
			});
		...
		refreshGroupDiscountList: function(){
			$('select#discount').change();
		},

	...
	<div class="col-md-2 mobile-settings-view"> 
		<div class="portlet grey box">
			<div class="portlet-title">
				<div class="caption"><i class="icon-globe"></i><small><small>Audit Report</small></small></div>
			</div>
			<div class="portlet-body" style="max-height:470px; overflow-y:auto;">
				...
				<input type="date" id="start-date" value="<?php echo date("Y-m-d" , mktime(0,0,0, date("n"), 1, date("Y") ) ); ?>" class="form-control date-range" style="font-size: 12px; padding: 2px 5px; height: 28px; font-weight: bold;" />
				<br />
				<label style="font-weight:bold; font-size:11px;">End Date</label>
				<input type="date" id="end-date" value="<?php echo date("Y-m-d", mktime(23,59,59, date("n"), date("t") , date("Y") ) ); ?>" class="form-control date-range" style="font-size: 12px; padding: 2px 5px; height: 28px; font-weight: bold;" />
				<hr />
				<?php } ?>
				
				<?php if( ! ( isset( $data[ 'hide_item' ] ) && $data[ 'hide_item' ] ) ){ ?>
				<?php if( ! ( isset( $data[ 'hide_staff' ] ) && $data[ 'hide_staff' ] ) ){ ?>
				<label style="font-weight:bold; font-size:11px;">Staff</label>
				...
				<br />
				<?php } ?>
				<label style="font-weight:bold; font-size:11px;">Report Time</label>
				...
				<?php if( ! $mobile ){ ?>
					<a href="#" budget-id="-" month-id="-" operator-id="<?php if( isset( $data[ 'staff' ] ) && $data[ 'staff' ] )echo $data[ 'staff' ]; else echo "-"; ?>" department-id="-" skip-title="1" function-id="activate_report_year" function-class="hotel_checkin" function-name="<?php if( isset( $data[ 'report_action' ] ) && $data[ 'report_action' ] ){ echo $data[ 'report_action' ]; } ?>" id="activate_report_year" class="btn btn-block btn-success custom-action-button">Generate Report</a>
				<?php }else{ ?>

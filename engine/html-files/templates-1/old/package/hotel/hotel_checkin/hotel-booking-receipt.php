...
<?php
	include dirname( dirname( dirname( dirname( __FILE__ ) ) ) ) . "/globals/invoice-css.php";
		...
		$booked = 1;
		$booking_status_text = "";
		$booking_status_array = get_hotel_room_booking_status();
		$bk = array();
		
		if( isset( $data["room_types"] ) && is_array( $data["room_types"] ) && ! empty( $data["room_types"] ) ){
			if( count( $data["room_types"] ) == 1 ){
				$booking_status = $data["room_types"][0]["status"];
				if( isset( $booking_status_array[ $booking_status ] ) )$booking_status_text = $booking_status_array[ $booking_status ];
			}else{
				foreach( $data["room_types"] as $items ){
					$booking_status = $items["status"];
					switch( $items["status"] ){
					case "checked_out":
					case "checked_in":
					case "cancelled":
					...
					if( isset( $booking_status_array[ $booking_status ] ) ){
						if( ! isset( $bk[ $booking_status ] ) ){
						$booking_status_text .= $booking_status_array[ $booking_status ] . ", ";
						$bk[ $booking_status ] = 1;
						}
					}
				}
		...
		$delete_action = "?module=&action=hotel_checkin&todo=delete_app_hotel_checkin";
		if( $booked ){
			$show_buttons = 1;
			$delete_caption = "Cancel Reservation";
			$delete_action = "?module=&action=hotel_room_checkin&todo=update_status_to_cancelled_from_report";
			
			$pay_button = '';
			if( $amount_paid ){
				$pay_button = '<a href="#" class="btn btn-sm red hidden-print custom-single-selected-record-button" action="?module=&action=sales&todo=refund_customer3&customer=' . $data["event"]["main_guest"] . '" mod="' . $data["event"]["id"] . '" title="Refund Guest '.number_format( $amount_paid , 2 ).'" override-selected-record="' . ( $amount_paid ) . '">Refund Guest</a>';
			}
		}
	...
	<div class="container" id="invoice-container">
		...
		<div class="row ">
		   <div class="col-xs-5 ">
			  <h4>Guest: <span class="pull-right label label-info"><strong style="font-size:11px; color:#000;"><i class="icon-pushpin"></i> <?php echo $booking_status_text; ?></strong></span></h4>
			  <ul class="list-unstyled">
				 <li><strong><?php $main_guest = ""; $key = "main_guest"; if( isset( $data["event"][$key] ) )$main_guest = get_select_option_value( array( "id" => $data["event"][$key], "function_name" => "get_customers" ) ); echo $main_guest; ?></strong></li>
		   ...
		   <div class="col-xs-7 invoice-payment">
			  <div class="well payment-status-container">
				 ...
				 <ul class="list-unstyled">
					 ...
					 <li>&nbsp;</li>
					 <li><strong>Check In:</strong> <?php $key = "checkin_date"; if( isset( $data["event"][$key] ) )echo date( "d-M-Y H:i" , doubleval( $data["event"][$key] ) ); ?></li>
				  ...
				  <div class="btn-group btn-group-justified hidden-print">
					...
					<?php 
						if( $show_buttons ){ 
							echo $pay_button;
					?>
					<a class="btn btn-sm dark default custom-single-selected-record-button" override-selected-record="<?php $key = "id"; if( isset( $data["event"][$key] ) )echo $data["event"][$key]; ?>" action="<?php echo $delete_action; ?>" href="#"><?php echo $delete_caption; ?></a>
					<?php } ?>
		...
		<div class="row">
		   <div class="col-xs-6">
			<strong><small>Guest Name:</small></strong><br /><?php echo $main_guest; ?>
		   
		   <br /><br />
		   _____________<br /><strong><small>Signature:</small></strong>
		   </div>
		   <div class="col-xs-6">
			<strong><small>Staff Responsible:</small></strong><br /><?php $key = "modified_by"; if( isset( $data["event"][$key] ) )echo get_select_option_value( array( "id" => $data["event"][$key], "function_name" => "get_employees" ) ); ?>
		   <br /><br />
		   _____________<br /><strong><small>Signature:</small></strong>
		   </div>
		</div>

...
<?php
	...
	if( ! empty( $report_data ) ){
		...
		foreach( $report_data as $sval ){
			...
			if( doubleval( $sval["amount_paid"] ) > doubleval( $sval["amount_refund"] ) ){
			...
			$body .= '<tr '.$ocuppied.'>';
				...
				$pay_button = '';
				if( doubleval( $sval["amount_paid"] ) > doubleval( $sval["amount_refund"] ) ){
					$pay_button = '<br /><br /><a href="#" class="btn btn-xs red hidden-print custom-single-selected-record-button" action="?module=&action=sales&todo=refund_customer2&customer=' . $sval["main_guest"] . '" mod="' . $sval["id"] . '" title="Refund Guest" override-selected-record="' . ( $sval["amount_paid"] ) . '">Refund</a>';
				}

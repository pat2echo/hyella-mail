				...
				*/
				...
				$body .= '<tr '.$ocuppied.'>';
					...
					if( $sval[ 'type' ] == "sales" ){
						...
						$income_text = number_format( $income , 2 );
					}else{
						$out = $sval["out"];
						$dis = $sval["dis"];
						...
						if( $sval[ 'status' ] == "cancelled" ){
							$refund_amount = $tpaid;
							$income_text = '<span class="label label-danger">rebate</span>';
						}
						...
						if( ( ! isset( $group[ $sval["booking_ref"] ] ) ) && $refund_amount > 0 ){
							$refund = '<a href="#" class="custom-single-selected-record-button btn-xs btn red" action="?module=&action=sales&todo=refund_customer2&customer=' . $guest . '&store='.$sval["hotel"].'&callback=nwReportView.regenerateReport" title="Click to Refund Guest" mod="' . $sval["room_checkin_id"] . '" override-selected-record="' . round( $refund_amount, 2 ) . '">Refund<br />'.number_format( $refund_amount , 2 ). '</a>';
						}
					...
					$body .= '<td class="company alternate">' . $income_text . '</td>';
					$body .= '<td>' . $due_text . '</td>';
					$body .= '<td>' . $refund . '</td>';

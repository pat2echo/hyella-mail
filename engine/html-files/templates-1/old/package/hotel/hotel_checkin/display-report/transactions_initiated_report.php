...
<?php
	...
	if( ! empty( $report_data ) ){
		...
		foreach( $report_data as $sval ){
			...
			case "day":
				$pass = 0;
				if( ( doubleval( date("H.i", $sval["creation_date"] ) ) < $time && doubleval( date("H.i", $sval["creation_date"] ) ) > $etime ) ){
					$pass = 1;
				}
			break;
			case "night":
			...
			$body .= '<tr '.$ocuppied.'>';
				...
				$body .= '<td class="company">' . date( "H:i" , $sval["creation_date"] ) . '</td>';
				$body .= '<td>' . date( "H:i" , $sval["modification_date"] ) . '</td>';

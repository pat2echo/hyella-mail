	...
	if( isset( $data[ "items" ] ) && is_array( $data[ "items" ] ) && isset( $data[ "fields" ] ) && is_array( $data[ "fields" ] ) && isset( $data[ "labels" ] ) && is_array( $data[ "labels" ] ) ){
		...
		foreach( $data[ "items" ] as $item => $item_data ){
			...
			if( ! isset( $item_data["id"] ) )
				$dass = get_customer_call_log_details( array( "id" => $item ) );
			else
				$dass = $item_data;
?>
<hr />
...
<button class="btn dark custom-single-selected-record-button" action="?module=&action=customer_call_log&todo=new_popup_form_in_popup" override-selected-record="<?php echo $dass["customer"]; ?>">Add Another Item ?</button>
	<?php } ?>
<?php } ?>
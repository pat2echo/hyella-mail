...
<?php
	...
	if( ! empty( $report_data ) ){
		...
		foreach( $report_data as $sval ){
			...
			case "charge_to_room":
				$income = 0;
				$extra_info = '<br /><small>' . $sval["comment"] . '</small>';
			...
			$body .= '<tr>';
				...
				$body .= '<td><strong><a href="#" class="custom-single-selected-record-button" action="?module=&action=sales&todo=view_invoice_app1&hide=1" title="View Invoice / Receipt" override-selected-record="'.$sval["id"].'">#' . $sval["serial_num"] . '-' . $sval["id"] . '</a></strong>'.$extra_info.'</td>';

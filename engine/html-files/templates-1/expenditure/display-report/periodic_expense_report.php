...
<?php
	...
	if( ! empty( $report_data ) ){
		...
		foreach( $report_data as $sval ){
			$body .= '<tr>';
				...
				$body .= '<td class="landscape-only"><strong>' . ( isset( $vendor[ $sval["vendor"] ] )?$vendor[ $sval["vendor"] ]:$sval["vendor"] ) . '</strong></td>';
	...
	?>
	<tr><td colspan="2" rowspan="<?php echo count( $total ) + 1; ?>"><h4><strong id="e-report-title"><?php echo $title; ?></strong></h4><br /><p><?php echo $subtitle; ?></p></td><td colspan="4" ><h5>SUMMARY</h5></td></tr>
	<?php
	foreach( $total as $key => $val ){
		?>
		<tr><td colspan="1"><strong><?php echo ucwords( $key ); ?></strong></td><td colspan="3" ><?php echo $val; ?></td></tr>
		<?php
	}
?>
	<tr>
		<th rowspan="2" class="company">Date</th>
		<th rowspan="2" class="landscape-only">Description</th>
		<th colspan="3">Expenditure</th>
		<th rowspan="2" class="landscape-only">Vendor / Supplier</th>
	</tr>
	<tr>
		<th>Amount Due</th>
		<th>Amount Paid</th>

	...
	<div class="col-md-2 mobile-settings-view"> 
		<div class="portlet grey box">
			<div class="portlet-title">
				<div class="caption"><i class="icon-globe"></i><small><small>Teller Report</small></small></div>
			</div>
			<div class="portlet-body" style="max-height:470px; overflow-y:auto;">
				...
				<?php if( ! $mobile ){ ?>
					<a href="#" budget-id="-" month-id="-" operator-id="-" department-id="-" skip-title="1" function-id="activate_report_year" function-class="bank_transaction" function-name="<?php if( isset( $data[ 'report_action' ] ) && $data[ 'report_action' ] ){ echo $data[ 'report_action' ]; } ?>" id="activate_report_year" class="btn btn-block btn-success custom-action-button">Generate Report</a>
				<?php }else{ ?>
					
					<a href="#" budget-id="-" month-id="-" operator-id="-" department-id="-" skip-title="1" function-id="activate_report_year" function-class="transactions" function-name="<?php if( isset( $data[ 'report_action' ] ) && $data[ 'report_action' ] ){ echo $data[ 'report_action' ]; } ?>" id="activate_report_year" class="btn btn-block btn-success custom-action-button" onclick="$.fn.pHost.showMobileMainView();">Generate Report</a>
				<?php } ?>
			</div>
		</div>

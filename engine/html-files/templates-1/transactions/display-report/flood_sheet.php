	...
	if( ! empty( $report_data ) ){
		...
		foreach( $report_data as $key => $sval ){
			...
			case "expenses":
				...
				if( ! isset( $r[ $store ][ $sval["reference"] ][ "total" ] ) ){
					...
					$r[ $store ][ $sval["reference"] ][ "id" ] = '<a href="#" action="?module=&action=transactions&todo=view_invoice" class="custom-single-selected-record-button" override-selected-record="'.$sval["id"].'">' . $sval["id"] . '</a>';
				}
				if( $sval["type"] == "debit" )
			...
			case "cost_of_goods_sold":
				...
				$acc_details["title"] = 'Cost of Sales';
		...
		foreach( $r as $key => $rv ){
			...
			foreach( $rv as $kv => $vvv ){
				$body .= '<tr><td>'. date("d-M-Y", doubleval( $vvv["date"] ) ) .'</td><td>'. $vvv["id"] .'</td><td class="item-desc">'. ucwords( $vvv["description"] ) .'</td><td class="item-amount">'. format_and_convert_numbers( $vvv["total"] , 4 ) .'</td></tr>';

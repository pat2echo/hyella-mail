		...
		foreach( $report_data as $key => $sval ){
			...
			case "damaged_items":
				...
				if( isset( $category[ $sval["account"] ] ) && $category[ $sval["account"] ] ){
					$acc_details["title"] = 'DMG: ' . $category[ $sval["account"] ];
				}else{
					$acc_details["title"] = 'DMG: Uncategorized Expenses';
				}
				*/
				$acc_details["title"] = 'Damage Items';
				$ckey = "expenses";
			break;
			case "cost_of_goods_sold":
				if( isset( $category[ $sval["account"] ] ) && $category[ $sval["account"] ] ){
					$acc_details["title"] = 'CGS: '. $category[ $sval["account"] ];
				}else{
					$acc_details["title"] = 'CGS: Uncategorized Expenses';
				}
				$ckey = "expenses";
			break;
			case "inventory_marketing_expense":
				/*
				if( isset( $category[ $sval["account"] ] ) && $category[ $sval["account"] ] ){
					$acc_details["title"] = 'MKT: ' . $category[ $sval["account"] ];
				}else{
					$acc_details["title"] = 'MKT: Uncategorized Expenses';
				}
				*/
				$acc_details["title"] = 'Marketing Expenses';
				$ckey = "expenses";
			break;

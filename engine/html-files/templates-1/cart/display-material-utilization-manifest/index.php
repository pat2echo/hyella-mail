				...
				<div >
					...
					<div class="row" style="margin-left:0; margin-right:0; ">
						...
						<div class="col-md-6" style="padding-right:0;">
							<button class="btn btn-lg red btn-block custom-single-selected-record-button" action="?module=&action=cart&todo=save_production_manifest" id="cart-finishXX">Save Manifest</button>
						</div>
					</div>
		...
		<div class="portlet box">
			<div class="portlet-body allow-scroll" style="background:transparent;">
					...
					<?php
						if( isset( $data['categories'] ) && is_array( $data['categories'] ) ){
							foreach( $data['categories'] as $key => $val ){
								?>
								<a href="#" class="category icon-btn catid-<?php echo md5( $key ); ?>" id="<?php echo $key; ?>">
									<i class="icon-th"></i>
				...
				<div id="stocked-items" class="row">
					...
					<style type="text/css">
					#cart-category-materials .category{
					...
					<?php
						...
						if( isset( $data['stocked_items'] ) && is_array( $data['stocked_items'] ) ){
							...
							foreach( $data['stocked_items'] as $sval ){
								...
								//if( $sval["type"] == "service" || ! $q )continue;
								//if( $sval["type"] == "raw_materials" && ! $q )continue;
								
								$non_empty_categories[ $sval["category"] ] = "catid-" . md5( $sval["category"] );
								?>
								<div class="col-md-4 col-sm-3 cart-item cart-item-select <?php echo $sval["category"]." ".$sval["barcode"] . " " . $sval["type"]; ?>" id="<?php echo $sval["item"]; ?>" data-type="<?php echo $sval["type"]; ?>" data-max="<?php echo $q; ?>" data-price="<?php if( $sval["type"] == "raw_materials" )echo $sval["cost_price"]; else echo $sval["selling_price"]; ?>" data-cost="<?php echo $sval["cost_price"]; ?>">
									<div class="item-container">
							...
							if( ! empty( $non_empty_categories ) ){
								$cartegory_ids = implode( ",#cart-category-materials a.", $non_empty_categories );
								?>
								<style type="text/css">
									<?php echo "#cart-category-materials a." . $cartegory_ids; ?>{
		...
		<div style="background:transparent !important; border:none !important;" class="grey box portlet <?php if( ! ( isset( $data['mobile_enabled'] ) && $data['mobile_enabled'] ) ){ ?>grey box<?php } ?>">
			<div class="tabbable-custom nav-justified">
				<ul class="nav nav-tabs nav-justified">
				   <li class="active"><a href="#recent-expenses" onclick="nwProduction.specifyMaterials();" data-toggle="tab">Specify Raw Materials</a></li>
				   <li class="no-utilization"><a href="#extra-cost" data-toggle="tab">2. Extra Cost</a></li>
				   <li class="no-utilization"><a href="#recent-goods" onclick="nwProduction.specifyGoods();" data-toggle="tab">3. Specify Products</a></li>
				</ul>
				<div class="tab-content" style="background:transparent !important;">
				   <div class="allow-scroll-1 tab-pane active" id="recent-expenses">
					...
					<div class="shopping-cart-table">
						...
						<div class="input-group no-utilization">
						 <span class="input-group-addon" style=" font-size: 18px; line-height: 1.5;">Cost of Production</span>
						...
						<div class="row">
							...
							<div class="col-md-6">
								<a class="no-utilization btn btn-lg green btn-block" onclick="nwProduction.specifyExtraCostButtonClick(); return false;" href="#">Proceed to Next Step</a>
								<button class="btn btn-lg red btn-block custom-single-selected-record-button" action="?module=&action=cart&todo=save_material_utilization_manifest" id="cart-finish">Save Manifest</button>
							</div>
						</div>

...
$( document ).ready( function() {
	...
	function select_record_click_event($clicked_element_parent, $clicked_element, $clicked_element_group_selector, shiftctrlKey ){
		...
		*/
		...
		if($clicked_element.hasClass('row_selected')){
			...
			if( single_selected_record && record_click_by_user ){
	...
	function bind_quick_print_function(){
		...
		.live('click', function(e){
			...
			if( $form.find('input[name="report_template"]') && $form.find('input[name="report_template"]').val() )r_template = $form.find('input[name="report_template"]').val();
		...
		if( tmp_data && Object.getOwnPropertyNames( tmp_data ).length && tmp_data.highchart_data && tmp_data.highchart_container_selector ){
			...
			.on("click", function(e){
				...
				if( $chart ){
					...
					var dataString = 'type=image/jpeg&filename=exported_chart&width=600&svg='+svg;
					...
					$.ajax({
						...
						success: function( data ){
							//console.log( data );
							if( data && data.status && data.status == "imaged" ){
								var data = {theme:'alert-success', err:'Successfully Exported Chart', msg:'<a href="'+ pagepointer + 'files/charts/exported_chart.jpg" class="btn blue" target="_blank">Download Image File</a>', typ:'jsuerror' };
							}else{
								var data = {theme:'alert-danger', err:'Chart Export Failed', msg:'Please try again', typ:'jsuerror' };
							}
							display_notification( data );
							$("button.export-chart").text("Export Chart");
						}
					});
				}
			});
